...
# Store JSON event canonically as UTF-8
...
# Hash the current canonical event chained with previous event hash
...
def verify_chain(
    ...
    for idx, ev in enumerate(events):
        if ev.get("seq") != expected_seq:
            return ChainResult(
                False, idx, f"seq {ev.get('seq')} != expected {expected_seq}"
            )
