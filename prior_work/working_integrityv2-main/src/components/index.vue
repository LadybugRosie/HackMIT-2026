...
const localeConfig = $ref({
  'zh-CN': cnConfig,
  'en-US': enConfig,
  'ar-KSA': enConfig, // Fallback to English config for now
  'ja-JP': enConfig, // Fallback to English config for now
  'ur-IN': enConfig, // Fallback to English config for now
  'hi-IN': enConfig, // Fallback to English config for now
  'kn-IN': enConfig, // Fallback to English config for now
  'ta-IN': enConfig, // Fallback to English config for now
  'te-IN': enConfig, // Fallback to English config for now
  'sp-SPA': enConfig, // Fallback to English config for now
})
...
const setLocale = (parmas) => {
  if (!['zh-CN', 'en-US', 'ar-KSA', 'ja-JP', 'ur-IN', 'hi-IN', 'kn-IN', 'ta-IN', 'te-IN', 'sp-SPA'].includes(parmas)) {
    throw new Error('"parmas" must be one of "zh-CN", "en-US", "ar-KSA", "ja-JP", "ur-IN", "hi-IN", "kn-IN", "ta-IN", "te-IN", or "sp-SPA".')
  }
