<template>
  <div v-if="!page.preview.enabled" class="status-bar">
    <div class="bar-left">
      ...
      <!-- <tooltip :content="t('poweredBy')">
        <t-button
          ...
          variant="text"
          size="small"
          :href="`https://editor.umodoc.com/${['zh-CN'].includes(i18n.global.locale.value) ? 'cn' : ['sp-SPA'].includes(i18n.global.locale.value) ? 'es' : 'en'}/docs`"
          target="_blank"
        >
          <icon name="home-page" />
        </t-button>
    ...
    <div class="bar-right">
      <!-- Integrity Pill - gated by teacher's tool settings + hidden from students (new mode) -->
      <tooltip v-if="showLiveIntegrity" content="This is typing provenance only — not a final trust score.">
        <div
          style="background: white; border: 2px solid; border-radius: 8px; padding: 6px 12px; margin-right: 10px; cursor: pointer; font-weight: bold;"
          :style="{ borderColor: integrityScore === null ? '#9e9e9e' : integrityScore >= 70 ? '#4caf50' : integrityScore >= 50 ? '#ff9800' : '#f44336' }"
          @click="toggleIntegrityPanel"
        >
          <span style="font-size: 14px; color: #333;">TYPING: </span>
          <span
            style="font-size: 16px; font-weight: bold;"
            :style="{ color: integrityScore === null ? '#9e9e9e' : integrityScore >= 70 ? '#4caf50' : integrityScore >= 50 ? '#ff9800' : '#f44336' }"
          >
      ...
      <!-- Fact Check / Hallucination Detection Button -->
      <div 
        style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 8px; padding: 6px 14px; margin-right: 10px; cursor: pointer; font-weight: 600; color: white; font-size: 13px; display: flex; align-items: center; gap: 6px;"
        @click="toggleFactCheckPanel"
        title="Detect AI hallucinations, verify claims, DOIs and URLs"
      >
        <span>🔍</span>
  ...
  <!-- Integrity Panel - gated by teacher's tool settings + hidden from students (new mode) -->
  <IntegrityPanel
    v-if="showLiveIntegrity"
    :is-open="showIntegrityPanel"
    @close="showIntegrityPanel = false"
  />
  
  <!-- Fact Check Panel -->
  <FactCheckPanel 
    ref="factCheckPanelRef"
    :is-open="showFactCheckPanel" 
    @close="showFactCheckPanel = false" 
  />
</template>
...
import FactCheckPanel from '@/components/integrity/FactCheckPanel.vue'
import { integrityLegacyMode } from '@/composables/feature-flags'

const { container, options, page, editor, integrity, assignmentToolSettings } = useStore()
const $document = useState('document')
// Live in-editor integrity is a SINGLE typing-trust score (pill + panel), not
// independent signals — so in new mode it is hidden from EVERYONE (students and
// teachers/authors alike). Restored only when the legacy production switch is on.
const showLiveIntegrity = computed(() =>
  assignmentToolSettings.value.analyze_integrity_enabled && integrityLegacyMode.value
)

// Integrity panel state
const showIntegrityPanel = ref(false)
const showFactCheckPanel = ref(false)
const factCheckPanelRef = ref(null)

// Integrity computed properties - with reactive updates
const integrityScore = computed(() => {
  const score = integrity?.value?.scores?.trust
  if (score === null || score === undefined) return null
  return score
})
const integrityClass = computed(() => {
  const score = integrityScore.value
  if (score === null) return 'integrity-pending'
  if (score >= 70) return 'integrity-high'
  if (score >= 50) return 'integrity-medium'
  return 'integrity-low'
})

const integrityTooltip = computed(() => {
  if (integrityScore.value === null) return 'Trust score not yet analyzed — click Analyze Integrity'
  const flags = integrity?.value?.flags || []
  if (flags.length > 0) {
    return `Trust: ${integrityScore.value}% - ${flags.join(', ')}`
  }
  return `Trust Score: ${integrityScore.value}%`
})

const toggleIntegrityPanel = () => {
  showIntegrityPanel.value = !showIntegrityPanel.value
}

const toggleFactCheckPanel = () => {
  showFactCheckPanel.value = !showFactCheckPanel.value
}

// Listen for integrity updates
// CRITICAL: Do NOT overwrite scores from periodic sync events
// Only the Analyze button should update scores
onMounted(() => {
  window.addEventListener('integrity-update', (e) => {
    if (integrity.value) {
      // CRITICAL FIX: Preserve existing scores and analysis data
      // Never let periodic sync overwrite analyzed results
      const preservedData = {
        scores: integrity.value.scores,
        mix: integrity.value.mix,
        sessionMix: integrity.value.sessionMix,
        lastAnalyzed: integrity.value.lastAnalyzed,
        externalPastesFound: integrity.value.externalPastesFound,
        retypedExternalDetected: integrity.value.retypedExternalDetected,
        analysis: integrity.value.analysis,
        flags: integrity.value.flags
      }
      
      // Only merge non-score data from the event
      const safeDetail = { ...e.detail }
      delete safeDetail.scores
      delete safeDetail.mix
      
      integrity.value = {
        ...integrity.value,
        ...safeDetail,
        // Always preserve analyzed data
        ...preservedData
      }
    }
  })
...
const langs = [
  ...
  { content: '🇸🇦 العربية', value: 'ar-KSA' },
  { content: '🇯🇵 日本語', value: 'ja-JP' },
  { content: '🇮🇳 اردو', value: 'ur-IN' },
  { content: '🇮🇳 हिंदी', value: 'hi-IN' },
  { content: '🇮🇳 ಕನ್ನಡ', value: 'kn-IN' },
  { content: '🇮🇳 தமிழ்', value: 'ta-IN' },
  { content: '🇮🇳 తెలుగు', value: 'te-IN' },
  { content: '🇪🇸 Español', value: 'sp-SPA' },
]
