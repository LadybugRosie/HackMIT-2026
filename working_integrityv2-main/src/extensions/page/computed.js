...
export const defaultNodesComputed = {
  ...
  [TABLE_ROW]: (splitContex, node, pos, parent, dom) => {
    ...
    // Check if we're already at the page boundary (no space left)
    if (splitContex.isOverflow(0)) {
      // If this is not the first row and we have no space, break before this row
      if (parent?.firstChild !== node) {
        splitContex.setBoundary(pos, chunks.length - 1)
        return false
      }
      // If this is the first row and no space, break before the table
      splitContex.setBoundary(chunks[chunks.length - 2][2], chunks.length - 2)
      return false
    }
    ...
    if (splitContex.isOverflow(pHeight)) {
      // If this single row is taller than the entire page height, 
      // we must include it anyway (no choice)
      if (pHeight > splitContex.getHeight()) {
        splitContex.addHeight(pHeight)
        return false
      }
      
      // Row fits on a page, but not on current page
      if (parent?.firstChild == node) {
        // This is the first row of the table, break before the table
        splitContex.setBoundary(chunks[chunks.length - 2][2], chunks.length - 2)
      } else {
        // Break before this row, keeping previous rows on current page
        splitContex.setBoundary(pos, chunks.length - 1)
      }
      return false
    }
    
    // Row fits on current page, add its height
    splitContex.addHeight(pHeight)
    return false
  },
  [TABLE]: (splitContex, node, pos, parent, dom) => {
    ...
    // If table overflows the current page
    if (splitContex.isOverflow(pHeight)) {
      // If table is reasonably sized (< 80% of page height), move entire table to next page
      if (pHeight <= splitContex.getHeight() * 0.8) {
        // Set boundary before the table to move it to next page
        splitContex.setBoundary(pos, chunks.length - 1)
        return false
      }
      
      // Large table that exceeds 80% of page height: allow row-by-row splitting
      // Return true to continue processing table rows individually
      return true
    }
    
    // Table fits on current page, add its height
    splitContex.addHeight(pHeight)
    return false
  },
...
export class PageComputedContext {
  ...
  constructor(editor, nodesComputed, pageState, state) {
    ...
    this.startIndex = 0
    this.forceFullRecalculation = false
  }
  ...
  run() {
    ...
    if (initSplit || this.forceFullRecalculation) return this.initComputed()
    if (!inserting && deleting && selection.$head.node(1) === doc.lastChild)
      return this.tr
    if (inserting || deleting || this.forceFullRecalculation) {
